...
struct eap_method {
	...
	Boolean (*check)(void *priv,struct wpabuf *respData);
	// Caller MUST NOT free 'key_r'!
	void (*process)(void *priv,struct wpabuf *respData,
			int (*get_key)(void* ctx,const u8* key_id,int key_id_len,u8** key_r),void* ctx);
...
#define EAP_SERVER_METHOD_INTERFACE_VERSION 1
	...
	u8 * (*get_peer_identity)(void *priv,size_t *ident_len);
};
