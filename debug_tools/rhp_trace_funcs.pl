...
if( @ARGV < 1 ){
  ...
  while( <TRC> ){
	...
	if( @elms[$elm_i] =~ /##FUNC_ADDR_END##/ ){
	  ...
	  if( !$prtflg ){
	    print $elms[0]."  ".$rets[0]."()[".$elms2[0]."][".$paths[@paths-1]."]".$elms2[1];
	  }else{
	    print $rets[0]."()[".$elms2[0]."][".$paths[@paths-1]."]".$elms2[1];
	  }
	  $prtflg++;
        }
      }
	...
	print "  ";
      ...
      if( $bt_flag ){
        ...
        if( /\[/ ){
          ...
          @rets = split(/\n/,$ret);
          @paths = split(/\//,$rets[1]);
          print " ($elms[0])==> ".$rets[0]."() :\t[".$paths[@paths-1]."]\n";
